...
import {
  ...
  MongoId,
  Movimiento,
  MovimientoClaseSuelta,
  MovimientoInscripcion,
  MovimientoLiquidacionProfe,
  Taller,
} from '../../lib/api'
import { isInMonth } from '../../lib/utils'

export const endpoints = ['/api/alumnes/', '/api/talleres/', '/api/inscripciones/', '/api/movimientos/']

/** Hook para usar el backend (la API) */
export const useBackend = () => {
  ...
  const hydrateInsc: Hydrator<Inscripcion, Inscripcion> = (i: Inscripcion) => ({
    ...i,
    iniciada: new Date(i.iniciada),
    baja: i.baja ? new Date(i.baja) : undefined,
  })
  ...
  const addMember =
    <T>(setter: Setter<T>, hydrate: Hydrator<any, Elem<T>> = (t) => t) =>
    (elem: Elem<T>) => {
  ...
  const updateMember =
    <T>(coleccion: Coleccion<T>, setter: Setter<T>) =>
  ...
  const crearElem =
    <TPost, TRet>(endpoint: string, adder: (elem: Elem<TRet>) => void) =>
    async (elem: TPost) => {
      try {
        console.log('-------------------')
  ...
  const editarElem =
    <T>(endpoint: string, updater: ElemUpdater<T>) =>
  ...
  const eliminarElem =
    <T>(endpoint: string, deleter: (_id: string) => void) =>
    async (elem: Elem<T>) => {
      try {
        console.log('-------------------')
  ...
  const lkpMember =
    <T>(coleccion: Coleccion<T>) =>
    (_id: MongoId) =>
      find(coleccion, (e) => e._id == _id)!
  const lkpMembers =
    <T>(coleccion: Coleccion<T>, accesor: (e: T) => string) =>
    (_id: MongoId) =>
      coleccion.filter((e) => accesor(e) == _id)
  ...
  const lkpTalleresAlumne = (a: Alumne) =>
    lkpInscripcionesAlumne(a)
      .filter((i) => i.activa)
      .map((i) => lkpTallerInscripcion(i)) //.filter(t => t.activo)
  ...
  const lkpPagosInscripcion = (i: Inscripcion) =>
    lkpMembers(movimientos as any[], (m) => m.inscripcion)(i._id) as Movimiento[]

  const lkpInscripcionesTaller = (t: Taller) => lkpMembers(inscripciones, (i) => i.taller)(t._id)
  const lkpAlumnesTaller = (t: Taller) =>
    lkpInscripcionesTaller(t)
      .filter((i) => i.activa)
      .map((i) => lkpAlumneInscripcion(i))

  const lkpMovimientosTaller = (t: Taller) => lkpMembers(movimientos as any[], (m) => m.taller)(t._id) as Movimiento[]
  const lkpPagosTaller = (t: Taller) =>
    lkpMovimientosTaller(t).filter((p) => p.razon == 'clase suelta' || p.razon == 'inscripcion') as (
      | MovimientoClaseSuelta
      | MovimientoInscripcion
    )[]
  const lkpLiquidacionesTaller = (t: Taller) =>
    lkpMovimientosTaller(t).filter((p) => p.razon == 'liquidacion profe') as MovimientoLiquidacionProfe[]

  const lkpAsistenciasAlumneTallerMes = (ta: Taller, al: Alumne, mes: Date) =>
    asistencias.filter((a) => a.alumne == al._id && a.taller == ta._id && isInMonth(a.fecha, mes))
  const lkpInscripcionAlumneTaller = (a: Alumne, t: Taller) =>
    find(lkpInscripcionesActivasAlumne(a), (i) => i.taller == t._id)!
  ...
  const pullBackend = async () => {
    ...
    if (!raw_alumnes || !raw_talleres || !raw_inscripciones || !raw_movimientos) {
      console.error(`Error de red... alguna de las colecciones no llegó`)
      return
    }
