...
export default function Movimientos() {
  ...
  const entre_fechas = (m: Movimiento) =>
    (isBefore(m.fecha, hasta) && isAfter(m.fecha, desde)) || isEqual(m.fecha, desde) || isEqual(m.fecha, hasta)
  ...
  const movimientos_fuera_de_seleccion = [
    ...movimientos_entre_fechas.filter(otro_medio),
    ...movimientos_entre_fechas.filter(otra_direccion),
  ]
  ...
  const movimientos_en_pantalla = sortBy(
    [...movimientos_en_seleccion, ...(medio != 'todos' || direccion != 'ambas' ? [otros] : [])],
    (m) => m.fecha
  )

  const total_en_seleccion = movimientos
    .filter(entre_fechas)
    .filter(con_medio)
    .filter(con_direccion)
  ...
  return (
    <Lista titulo="Movimientos" bg="grilla">
      <div className="mx-6 flex justify-between w-72">
        <div className="mx-2">
          <p>Desde:</p>
          <DatePick fecha={desde} setFecha={setDesde} />
        </div>
        <div className="mx-2">
          <p>Hasta:</p>
          <DatePick fecha={hasta} setFecha={setHasta} />
        </div>
        <div className="mx-2">
          <p>Medios:</p>
      ...
      <Carta>
        ...
        <CartaBalance movimientos={movimientos_en_pantalla} />
      </Carta>
    </Lista>
  )
}
