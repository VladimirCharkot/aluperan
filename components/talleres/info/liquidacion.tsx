...
import { format_curr, isInMonth, nombres_meses } from '../../../lib/utils'
...
export const Liquidacion = ({ taller, mes }: LiquidacionProps) => {
  ...
  const balance_liquidacion = (mes: Date) => {
    const total_recaudado_mes = lkpPagosTaller(taller)
      .filter(
        (p) =>
    ...
    <>
      ...
      <div className=" bg-indigo-50/90 p-4 my-10 rounded-xl">
        ...
        <Table className="my-4">
          ...
          <TableBody>
            <TableRow>
              <TableCell>{format_curr(total)}</TableCell>
              <TableCell>{format_curr(a_liquidar_profe)}</TableCell>
              <TableCell>{format_curr(a_liquidar_alupe)}</TableCell>
            </TableRow>
          </TableBody>
        </Table>
        ...
        <div className="flex flex-col justify-center items-center mt-4">
          <p className={`text-sm mx-5 font-bold`}>
            ...
            <span className={`${pendiente ? 'text-rose-500' : 'text-emerald-500'}`}>
              {pendiente ? '❌ Pendiente' : '✅ Hecha'}
            </span>
          </p>
          {pendiente && <Boton texto="Liquidar" color="indigo" onClick={() => setViendoModalLiquidacion(true)} />}
        </div>
      </div>
    </>
  )
