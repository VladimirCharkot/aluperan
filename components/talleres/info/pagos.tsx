...
import { format_curr, isInMonth } from '../../../lib/utils'
...
export const Pagos = ({ taller, mes }: PagosProps) => {
  ...
  const pagosMes = lkpPagosTaller(taller).filter(
    (p) =>
      (p.razon == 'clase suelta' && isInMonth(p.ocasion, mes)) || (p.razon == 'inscripcion' && isInMonth(p.mes, mes))
  )
  ...
  const pagosInactivas = pagosInscripciones.filter((p) =>
    inscripcionesInactivas.map((i) => i._id).includes(p.inscripcion)
  )
          ...
          {(verInactivas ? inscripciones : inscripcionesActivas).flatMap((insc) => {
            ...
            if (pagosInsc.length === 0) {
              return (
                <TableRow key={insc._id}>
                  ...
                  <TableCell className={`text-center ${alum.ficha ? 'text-emerald-500' : 'text-red-500'}`}>
                    {ficha}
                  </TableCell>
            ...
            return pagosInsc.map((pago, idx) => (
              <TableRow key={`${insc._id}-${idx}`}>
                ...
                <TableCell className="text-right">{format_curr(pago.monto)}</TableCell>
          ...
          {pagosClasesSueltas.map((p, idx) => {
            ...
            return (
              <TableRow key={`clase-suelta-${idx}`}>
                ...
                <TableCell className="text-center">{asistencias}</TableCell>
                <TableCell className={`text-center ${alumne.ficha ? 'text-emerald-500' : 'text-rose-500'}`}>
                  {alumne.ficha ? '✓' : '✗'}
                </TableCell>
                <TableCell className="text-right">{format_curr(p.monto)}</TableCell>
          ...
          {!verInactivas && pagosInactivas.length > 0 && (
            <TableRow className="">
              <TableCell align="center">
                <Icon className="text-rose-500" icon={'icomoon-free:blocked'} />
              </TableCell>
