...
export const diasMesTaller = (t: Taller, mes: Date) => {
  ...
  return range(daysInMonth)
    .map((n) => set(mes, { date: n + 1 }))
    .filter((d) => some(checks.map((check) => check(d))))
}
...
const formateadorCurrency = new Intl.NumberFormat('es-AR', {
  style: 'currency',
  currency: 'ARS',
  minimumFractionDigits: 0,
})

export const format_curr = (amnt: number) => formateadorCurrency.format(amnt)

interface Periodo {
  desde: Date
  hasta: Date
}
export const isBetween = (d: Date, periodo: Periodo) =>
  (isAfter(d, periodo.desde) && isBefore(d, periodo.hasta)) || isEqual(d, periodo.desde)
